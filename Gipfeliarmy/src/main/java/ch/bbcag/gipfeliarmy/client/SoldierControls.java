...
final class SoldierControls {
    ...
    enum Tab {
        ...
        Component label() {
            return Component.translatable("gipfeliarmy.army.screen.tab." + this.name().toLowerCase(java.util.Locale.ROOT));
        }
    }
    ...
    SoldierControls(Consumer<AbstractWidget> add, int x, int y, UUID soldier, boolean commander, int posts, Tab initial) {
        ...
        for (int index = 0; index < roles.length; index++) {
            ...
            Button button = Button.builder(role.roleName(), b -> this.send(SoldierAction.KIT, role.token()))
                    ...
                    .tooltip(Tooltip.create(Component.translatable("gipfeliarmy.army.soldier.kit.hover", role.stack().getHoverName())
        ...
        Button disarm = Button.builder(Component.translatable("gipfeliarmy.army.soldier.disarm"), b -> this.send(SoldierAction.DISARM, ""))
        ...
        for (int index = 0; index < suits.length; index++) {
            ...
            Button button = Button.builder(suit.displayName(), b -> this.send(SoldierAction.ARMOUR, suit.token()))
                    .bounds(x + (index % 2) * (HALF + 4), top + (index / 2) * ROW, HALF, HEIGHT)
                    .tooltip(Tooltip.create(Component.translatable("gipfeliarmy.army.soldier.armour.hover")))
                    .build();
            this.armour.add(button);
            add.accept(button);
        }
        Button strip = Button.builder(Component.translatable("gipfeliarmy.army.soldier.strip"), b -> this.send(SoldierAction.STRIP, ""))
        ...
        this.camoButton = Button.builder(Component.empty(), b -> this.send(SoldierAction.CAMO, this.camo.next().token()))
                ...
                .tooltip(Tooltip.create(Component.translatable("gipfeliarmy.army.soldier.camo.hover")))
                .build();
        ...
        this.squadButton = Button.builder(Component.empty(), b -> this.send(SoldierAction.COLOUR,
                        (this.current == null ? "camo" : this.current.getName()) + (this.squad % ch.bbcag.gipfeliarmy.entity.GipfaeliSoldier.SQUADS_PER_COLOUR + 1)))
                .bounds(x, cursor, WIDTH, HEIGHT)
                .tooltip(Tooltip.create(Component.translatable("gipfeliarmy.army.soldier.squad.hover")))
                .build();
        ...
        this.promote = Button.builder(Component.empty(), b -> this.send(commanderNow() ? SoldierAction.DEMOTE : SoldierAction.PROMOTE, ""))
                .bounds(x, cursor, WIDTH, HEIGHT)
                .tooltip(Tooltip.create(Component.translatable("gipfeliarmy.army.soldier.promote.hover")))
                .build();
        ...
        for (int number = 1; number <= Math.min(posts, perRow * 2); number++) {
            ...
            Button button = Button.builder(Component.translatable("gipfeliarmy.army.soldier.post", number), b -> this.send(SoldierAction.POST, argument))
                    .bounds(x + (slot % perRow) * (postWidth + 2), cursor + (slot / perRow) * ROW, postWidth, HEIGHT)
                    .tooltip(Tooltip.create(Component.translatable("gipfeliarmy.army.screen.post.hover", number)))
                    .build();
        ...
        this.dismiss = Button.builder(Component.translatable("gipfeliarmy.army.order.dismiss"), b -> this.onDismiss())
                .bounds(x, cursor, WIDTH, HEIGHT)
                .tooltip(Tooltip.create(Component.translatable("gipfeliarmy.army.screen.dismiss.hover")))
                .build();
    ...
    void update(boolean commander) {
        ...
        this.promote.setMessage(Component.translatable(commander ? "gipfeliarmy.army.soldier.demote" : "gipfeliarmy.army.screen.promote"));
    }

    // The camouflage button names the pattern it is wearing now.
    void camo(GipfaeliCamo camo) {
        this.camo = camo;
        this.camoButton.setMessage(Component.translatable("gipfeliarmy.army.screen.camo", camo.displayName()));
    }

    void squad(int squad) {
        this.squad = squad;
        this.squadButton.setMessage(Component.translatable("gipfeliarmy.army.screen.number", squad));
    }
    ...
    private void onDismiss() {
        if (this.confirmTicks > 0) {
            this.confirmTicks = 0;
            this.dismiss.setMessage(Component.translatable("gipfeliarmy.army.order.dismiss"));
            this.send(SoldierAction.DISMISS, "");
            return;
        }

        this.confirmTicks = CONFIRM_TICKS;
        this.dismiss.setMessage(Component.translatable("gipfeliarmy.army.screen.dismiss.confirm"));
    }
    ...
    void draw(GuiGraphicsExtractor graphics, net.minecraft.client.gui.Font font, int mouseX, int mouseY, @Nullable DyeColor current) {
        ...
        for (int index = 0; index <= DyeColor.VALUES.size(); index++) {
            ...
            if (mouseX >= sx && mouseX < sx + SWATCH && mouseY >= sy && mouseY < sy + SWATCH) {
                graphics.setTooltipForNextFrame(font, Component.translatable("gipfeliarmy.army.screen.colour.hover", GipfaeliArmy.colourName(colour)), mouseX, mouseY);
            }
        }
    }
