...
import ch.bbcag.gipfeliarmy.territory.TerritoryNetwork.Action;
import ch.bbcag.gipfeliarmy.territory.TerritoryNetwork.ActionRequest;
import ch.bbcag.gipfeliarmy.territory.TerritoryNetwork.ChunkInfoPayload;
import ch.bbcag.gipfeliarmy.territory.TerritoryNetwork.ChunkInfoRequest;
import ch.bbcag.gipfeliarmy.territory.TerritoryNetwork.MapPayload;
import ch.bbcag.gipfeliarmy.territory.TerritoryNetwork.MapRequest;
...
public final class TerritoryScreen extends Screen {
    private static final Identifier MAP_TEXTURE = Identifier.fromNamespaceAndPath(GipfeliArmyMod.MODID, "territory_map");
    ...
    public TerritoryScreen(@Nullable ChunkPos selected) {
        super(Component.translatable("gipfeliarmy.territory.title"));
        if (selected != null) {
    ...
    private void rebuildTexture(MapPayload payload) {
        ...
        if (texture == null || textureSize != size) {
            texture = new DynamicTexture("gipfeliarmy territory map", size, size, false);
            textureSize = size;
    ...
    private static MutableComponent text(String key, Object... args) {
        return Component.translatable("gipfeliarmy.territory.screen." + key, args);
    }
}
