...
public final class GipfaeliLauncher {
    ...
    public static boolean use(Player player, ItemStack stack, Level level) {
        if (!Config.on(Config.ENABLE_GIPFAELI) || !stack.is(GipfeliArmyMod.GIPFAELI_LAUNCHER.get())) {
            return false;
        }
    ...
    private static boolean fire(Player player, ItemStack stack, Level level) {
        ...
        if (!free && ammoSlot == NO_AMMO) {
            ...
            GipfaeliLock.readout(player, Component.translatable("gipfeliarmy.gipfaeli.empty"));
            return true;
        }
    ...
    static int findAmmoSlot(Player player) {
        ...
        for (int slot = 0; slot < inventory.getContainerSize(); slot++) {
            if (inventory.getItem(slot).is(GipfeliArmyMod.GIPFAELI.get())) {
                return slot;
            }
        }
