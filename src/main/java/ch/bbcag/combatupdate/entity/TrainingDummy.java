...
public final class TrainingDummy extends Mob implements Enemy {
    ...
    // Monster hunters leave it be. The army is the one exception: a soldier only goes for a dummy
    // when it has been ordered to, and testing the guns is a fair reason to order it.
    @SubscribeEvent
    public static void onChangeTarget(LivingChangeTargetEvent event) {
        if (event.getNewAboutToBeSetTarget() instanceof TrainingDummy && !(event.getEntity() instanceof GipfaeliSoldier)) {
