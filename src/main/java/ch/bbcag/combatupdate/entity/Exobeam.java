...
public final class Exobeam extends AbstractHurtingProjectile {
    ...
    // A streak of colour cycling through the spectrum along the stretch just flown, dense enough to
    // read as one line at speed rather than a row of dots.
    private void trail() {
        ...
        for (int i = 0; i < points; i++) {
            ...
            float hue = ((this.tickCount * points + i) * 0.03F) % 1.0F;
            int color = Mth.hsvToRgb(hue, 0.65F, 1.0F) & 0xFFFFFF;
            this.level().addParticle(new DustParticleOptions(color, 1.6F), at.x, at.y, at.z, 0.0, 0.0, 0.0);
        }
